...
Platform: 联想异构智算平台（容器环境 / 用户隔离 / GPU）
...
# ═══════════════════════════════════════════════════════════════════════════════
# 0. 最早诊断 — 确认 Python 已启动
# ═══════════════════════════════════════════════════════════════════════════════
_DIAG = []
def _stamp(msg):
    """写 stdout + 记录诊断日志，多级保障输出一定可见。"""
    _DIAG.append(msg)
...
_stamp(f"  Python: {sys.version.split()[0]}")
_stamp(f"  argv: {sys.argv}")

# ═══════════════════════════════════════════════════════════════════════════════
# 1. 实时输出 — 兼容非标准 stdout（容器 / 云平台常见）
# ═══════════════════════════════════════════════════════════════════════════════
...
for _stream in (sys.stdout, sys.stderr):
    if hasattr(_stream, 'reconfigure'):
        try:
            ...
            pass
    # 兜底：直接操作底层 buffer
    if hasattr(_stream, 'buffer') and hasattr(_stream.buffer, 'raw'):
        pass  # binary layer unbuffered via PYTHONUNBUFFERED=1

_stamp("stdout/stderr reconfigure done")
...
if not _HF_ENDPOINT:
    ...
    _stamp("  HF: direct connect (no mirror)")
...
# 关键：通过 FORCE_DEVICE 环境变量跳过可能挂死的 CUDA 检测
_FORCE = os.environ.get("FORCE_DEVICE", "").lower()
_stamp(f"  FORCE_DEVICE={'auto' if not _FORCE else _FORCE}")

_stamp("Importing torch...")
import torch
_stamp(f"  torch {torch.__version__} imported OK")

# CUDA 检测 — 设 FORCE_DEVICE=cpu 可跳过
def _safe_detect_device():
    if _FORCE in ("cpu",):
        return "cpu"
    if _FORCE in ("cuda", "gpu"):
        return "cuda"
    # 自动检测 — 有挂死风险，但这里已经 import torch 成功了
    try:
        if torch.cuda.is_available():
            _stamp(f"  CUDA available: {torch.cuda.get_device_name(0)}")
            return "cuda"
    except Exception as e:
        _stamp(f"  CUDA detection failed: {e}")
    return "cpu"

_DEVICE = _safe_detect_device()
...
from config import Config as _ConfigBase
...
# Monkey-patch Config 的设备检测，确保不会再次触发 CUDA 检测
_ConfigBase._safe_device = _DEVICE

class Config(_ConfigBase):
    """平台适配版 Config：设备由外部安全检测注入。"""
...
def run_experiment(config, train_dataset, val_dataset, test_dataset,
    ...
    logger.info(f"=== Experiment: {config.experiment_name} | Attack: {config.attack_type} ===")
    ...
    client_datasets = {cid: Subset(train_dataset, indices)
                       for cid, indices in partition.items()}
    client_sample_counts = {cid: len(indices) for cid, indices in partition.items()}
    ...
    num_mal = (int(config.num_clients * config.malicious_ratio)
               if config.attack_type != "none" else 0)
    malicious_ids = set(range(num_mal))

    utility_tracker = UtilityScoreTracker(alpha=config.utility_alpha)
    round_details, per_class_records = [], []
    ...
    for round_t in tqdm(range(config.num_rounds), desc=config.experiment_name):
        ...
        for cid in selected:
            ...
            if cid in malicious_ids:
                update, meta = _apply_attack(
                    ...
                    round_num=round_t + 1, dfr_sigma_est=dfr_sigma_est,
                    afr_state=afr_state, val_loss_t=val_loss_t,
                    trainable_keys=trainable_keys,
                )
                updates[cid] = update
        ...
        if afr_state is not None and val_loss_t is not None and afr_base_norms_this_round:
            afr_state.update(val_loss_t, float(np.mean(afr_base_norms_this_round)))
        ...
        per_class_sv = estimate_round_shapley_per_class(
            ...
            num_classes=config.num_classes, num_mc_samples=config.num_mc_samples,
            device=config.device, sample_counts=round_sample_counts,
        )
        ...
        new_sd, server.momentum_buffer = fedavg_aggregate(
            global_sd, updates, server_lr=server.current_server_lr,
        ...
        logger.info(f"Round {round_t:>2d}: acc={test_acc:.4f}  loss={test_loss:.4f}  selected={selected}")
        ...
        for cid in selected:
            ...
            round_details.append({
                ...
                "attack_type": config.attack_type, "round": round_t,
                "client_id": cid, "is_malicious": cid in malicious_ids,
            ...
            rec = {"experiment_name": config.experiment_name, "round": round_t,
                   "client_id": cid, "is_malicious": cid in malicious_ids}
...
def main():
    ...
    if _DEVICE == "cuda":
        ...
        _stamp("GPU optimizations: cudnn.benchmark + TF32")
    # ───────────────────────────────────────────────────────────────────


    base = Config(
        model_dir=os.environ.get("MODEL_DIR", ""),
        num_clients=10, num_rounds=50, local_epochs=2,
        local_lr=0.0005, server_lr=0.5,
        participation_ratio=0.8, batch_size=128,
        num_classes=20, val_ratio=0.1, iid=True,
        malicious_ratio=0.4, num_mc_samples=30,
        seed=42, results_dir="results",
    )
    base.device = _DEVICE  # 使用安全检测的设备
    ...
    logger.info(f"Data loaded — train={len(train_ds)}, val={len(val_ds)}, test={len(test_ds)}")
